...
"""Build triple-degradation samples from double parents and single supports."""
...
def resolve_double_input_dir(cfg: dict[str, Any], override: str | None = None) -> Path:
    """Resolve the double parent input directory."""

    if override:
        path = resolve_path(override, must_exist=True)
    else:
        stage_cfg = cfg.get("stages", {}).get("double", {}) or {}
        path = resolve_path(stage_cfg.get("output_dir") or Path(cfg["output_root"]) / "double", must_exist=True)
    assert path is not None
    return path


def resolve_triple_output_dir(cfg: dict[str, Any], override: str | None = None) -> Path:
    """Resolve the triple stage output directory."""

    if override:
        path = resolve_path(override)
    else:
        stage_cfg = cfg.get("stages", {}).get("triple", {}) or {}
        path = resolve_path(stage_cfg.get("output_dir") or Path(cfg["output_root"]) / "triple")
    assert path is not None
    return path


def triple_signature(parent: DegradationSample, support: DegradationSample) -> str:
    """Build an order-aware signature used to avoid duplicate regeneration."""

    return f"triple:{parent.sample_id}->{support.sample_id}"


def collect_triple_candidates(
    double_samples: list[DegradationSample],
    ...
    """Collect same-clean triple candidates."""

    singles = group_single_by_clean_and_degradation(single_samples)
    candidates: list[tuple[str, DegradationSample, DegradationSample]] = []
    combo_counts: dict[str, int] = {}
    parents = list(double_samples)
    rng.shuffle(parents)
    for parent in parents:
        by_deg = singles.get(parent.clean_image_id, {})
        existing_degs = set(parent.degradations)
        for support_deg, supports in sorted(by_deg.items()):
            if support_deg in existing_degs:
                continue
            combo = combo_key([*existing_degs, support_deg])
            if combo_filter is not None and combo not in combo_filter:
                continue
            if combo_counts.get(combo, 0) >= candidate_cap_per_combo:
                continue
            shuffled_supports = list(supports)
            rng.shuffle(shuffled_supports)
            for support in shuffled_supports:
                if combo_counts.get(combo, 0) >= candidate_cap_per_combo:
                    break
...
def generate_triple_one(
    *,
    ...
    """Generate one triple-degradation sample."""
    ...
    sample_id = safe_name(
        f"{parent.clean_image_id}@triple@{ordered_key(ordered_degradations)}@{uuid.uuid4().hex[:8]}",
        max_len=150,
    )
    ...
    meta = {
        ...
        "combo": combo,
        "lineage_signature": triple_signature(parent, support),
    ...
    append_jsonl(dirs["manifests"] / "triple_manifest.jsonl", meta)
    return meta


def build_triple_dataset(args: argparse.Namespace) -> dict[str, Any]:
    """Build triple-degradation data from double and single samples."""
    ...
    double_input_dir = resolve_double_input_dir(cfg, args.double_input_dir)
    output_dir = resolve_triple_output_dir(cfg, args.output_dir)
    stage_cfg = cfg.get("stages", {}).get("triple", {}) or {}
    max_images = args.max_images if args.max_images is not None else stage_cfg.get("max_images")
    per_combo_limit = args.per_combo_limit if args.per_combo_limit is not None else stage_cfg.get("per_combo_limit", 2000)
    ...
    double_samples = scan_stage_samples(double_input_dir, expected_stage="double")
    existing = scan_stage_samples(output_dir, expected_stage="triple") if output_dir.exists() else []
    ...
    candidates = collect_triple_candidates(double_samples, single_samples, rng, combo_filter, candidate_cap)
    summary: dict[str, Any] = {
        "stage": "triple",
        "config": str(resolve_path(args.config)),
        "source_single_root": str(source_single_root),
        "double_input_dir": str(double_input_dir),
        "output_dir": str(output_dir),
        "single_samples": len(single_samples),
        "double_samples": len(double_samples),
    ...
    for combo, parent, support in candidates:
        ...
        if counts.get(combo, 0) >= per_combo_limit:
            ...
            continue
        sig = triple_signature(parent, support)
        if sig in signatures:
            summary["skipped_existing_signature"] += 1
            continue
        meta = generate_triple_one(
            cfg=effect_cfg,
        ...
        print(
            f"[GENERATED] triple {summary['generated']} | combo={combo} | "
    ...
    dirs = stage_dirs(output_dir)
    write_summary(dirs["manifests"] / "triple_generation_summary.json", summary)
    print("[OK] triple degradation generation finished")
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    return summary


def parse_args() -> argparse.Namespace:
    """Parse direct triple builder arguments."""

    parser = argparse.ArgumentParser(description="Build STAR-Agent triple degradation dataset.")
    ...
    parser.add_argument("--double_input_dir", default=None)
...
def main() -> int:
    ...
    build_triple_dataset(parse_args())
    return 0
